...
async function onCreateNode(
  ...
  const markdownPreface = opts.markdownPreface || 'md//';

  // add YAML object as node
  ...
  if (_.isArray(parsedContent)) {
    parsedContent.forEach((obj, i) => {
      transformObject(
        opts.enableRemark ? recursiveMarkdownRender(obj) : obj,
        obj.id || createNodeId(`${node.id} [${i}] >>> YAML`),
        _.upperFirst(_.camelCase(`${node.name} Yaml`))
      );
    });
  } else if (_.isPlainObject(parsedContent)) {
    transformObject(
      opts.enableRemark ? recursiveMarkdownRender(parsedContent) : parsedContent,
      parsedContent.id || createNodeId(`${node.id} >>> YAML`),
